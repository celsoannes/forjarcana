...
if (!isset($_GET['impressora_id'])): ?>
    ...
    <?php foreach ($impressoras as $imp): ?>
      <div class="col-md-4 mb-3">
        <div class="card card-clickable" onclick="window.location='?pagina=produtos&acao=adicionar&impressora_id=<?= $imp['id'] ?>&tipo=<?= $imp['tipo'] ?>'">
          <div class="card-body">
    ...
    <a href="?pagina=produtos" class="btn btn-secondary mt-3">Voltar</a>
    <style>
...
if (isset($_GET['impressora_id']) && !isset($_GET['material_id'])):
  ...
  <div class="col-md-4 mb-3">
    <div class="card card-clickable"
         onclick="window.location='?pagina=produtos&acao=adicionar&impressora_id=<?= $_GET['impressora_id'] ?>&tipo=<?= $tipo ?>&material_id=<?= $mat['id'] ?>&material_tabela=<?= ($tipo === 'FDM' ? 'filamento' : 'resinas') ?>'">
      <div class="card-body">
...
<?php endforeach; ?>
    ...
    <a href="?pagina=produtos&acao=adicionar" class="btn btn-secondary mt-3">Voltar</a>
    <?php
    exit;
endif;
...
if (isset($_GET['impressora_id']) && isset($_GET['material_id'])):
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!$nome_produto) {
            ...
            $stmt = $pdo->prepare("INSERT INTO produtos (usuario_id, impressora_id, material_id, nome_produto, descricao, imagem) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->execute([
                $usuario_id,
                $_GET['impressora_id'],
                $_GET['material_id'],
    ...
    <form method="POST" enctype="multipart/form-data">
      ...
      <a href="?pagina=produtos&acao=adicionar&impressora_id=<?= $_GET['impressora_id'] ?>&tipo=<?= $tipo ?>" class="btn btn-secondary ms-2">Voltar</a>
    </form>
    <?php
endif;
?>