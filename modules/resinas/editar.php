...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  } else if (isset($_POST['remover_capa']) && $_POST['remover_capa'] === '1') {
    // Remove capa existente
    if (!empty($caminhoCapa)) {
      $capaFullPath = __DIR__ . '/../../' . $caminhoCapa;
      ...
      } else if (file_exists($capaFullPath)) {
        @unlink($capaFullPath);
      }
    }
    $caminhoCapa = null;
  ...
  if (!$nome || !$marca || !$cor || !$preco_kilo) {
    $erro = 'Preencha todos os campos obrigatórios.';
  } elseif ($link_compra !== '' && !filter_var($link_compra, FILTER_VALIDATE_URL)) {
    $erro = 'O link de compra informado não é um URL válido.';
  } else {
    try {
      $stmt = $pdo->prepare("UPDATE resinas SET nome = ?, marca = ?, cor = ?, preco_kilo = ?, link_compra = ?, capa = ?, ultima_atualizacao = NOW() WHERE id = ? AND usuario_id = ?");
      $stmt->execute([$nome, $marca, $cor, $preco_kilo, $link_compra, $caminhoCapa, $id, $usuario_id]);
        ...
        <div class="col-md-3">
          <div class="form-group h-100">
            ...
            <div id="capa-preview-area" class="border rounded bg-light position-relative" style="min-height: 470px; cursor: pointer;" onclick="document.getElementById('foto').click();">
              <img id="preview-capa" src="<?= !empty($caminhoCapa) ? htmlspecialchars($caminhoCapa) : '' ?>" alt="Pré-visualização da capa" class="img-fluid w-100 h-100<?= !empty($caminhoCapa) ? '' : ' d-none' ?>" style="min-height: 470px; object-fit: cover;">
              <button type="button" id="remove-capa-btn" class="btn btn-danger btn-sm rounded-circle<?= !empty($caminhoCapa) ? '' : ' d-none' ?>" style="position:absolute; top:8px; right:8px; width:28px; height:28px; padding:0; line-height:26px;" onclick="event.stopPropagation(); document.getElementById('remover-capa').value = '1'; document.getElementById('form-editar-resina').submit();">&times;</button>
              <div id="capa-placeholder" class="align-items-center justify-content-center text-muted<?= !empty($caminhoCapa) ? ' d-none' : '' ?>" style="position:absolute; top:0; right:0; bottom:0; left:0; display:flex;">
                Clique para selecionar a capa
              </div>
            </div>
            <input type="file" id="foto" name="foto" accept=".jpg,.png,.webp" style="display:none;">
            <input type="hidden" id="remover-capa" name="remover_capa" value="0">
          </div>
        </div>
        <div class="col-md-9">
          <div class="form-group position-relative">
            <label for="nome">Série/Atributo</label>
            <input type="text" class="form-control" id="nome" name="nome" required placeholder="Basic, Pro, Tough, ABS-Like, Flex" autocomplete="off" value="<?= htmlspecialchars((string) ($resina['nome'] ?? '')) ?>">
          ...
          <div class="form-group position-relative">
            ...
            <input type="text" class="form-control" id="marca" name="marca" required autocomplete="off" value="<?= htmlspecialchars((string) ($resina['marca'] ?? '')) ?>">
          ...
          <div class="form-group position-relative">
            ...
            <?php $corSelecionada = (string) ($resina['cor'] ?? ''); ?>
          ...
          <div class="form-group">
            ...
            <input type="number" step="0.01" class="form-control" id="preco_kilo" name="preco_kilo" required value="<?= htmlspecialchars((string) ($resina['preco_kilo'] ?? '')) ?>">
          </div>
          <div class="form-group">
            <label for="link_compra">Link de Compra do Produto</label>
            <input type="url" class="form-control" id="link_compra" name="link_compra" placeholder="https://www.loja.com/produto" value="<?= htmlspecialchars((string) ($resina['link_compra'] ?? '')) ?>">
          </div>
        </div>
      </div>
    </div>
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  if (inputFoto && previewImagem && capaPlaceholder && removeCapaBtn) {
    function renderizarCapaExistente() {
      // Se já existe uma capa (src preenchido e não vazio), mostrar imagem e botão
      if (previewImagem.getAttribute('src') && previewImagem.getAttribute('src').trim() !== '') {
    ...
    inputFoto.addEventListener('change', function () {
      ...
      if (!arquivo) {
        // Se não selecionou novo arquivo, mantém o preview da capa existente
        renderizarCapaExistente();
        return;
      }
